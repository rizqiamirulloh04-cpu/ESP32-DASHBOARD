...
void drawDashboard(int speed, int rpm, int batt) {
    ...
    for (int i = 0; i < 12; i++) {
        // Sudut mulai dari kiri-bawah (140 deg) ke kanan-bawah (400 deg)
        int angle = map(skala[i], 0, 120, 140, 400);
        float rad = angle * M_PI / 180.0;
        
        // Garis Skala (lebih panjang untuk kelipatan 20)
        ...
        // Posisi Angka yang dirapikan
        if (skala[i] % 20 == 0 || skala[i] == 0) {
            // Jarak teks dari pusat disesuaikan agar tidak menumpuk
